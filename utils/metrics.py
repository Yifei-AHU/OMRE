...
class Evaluator():
    ...
    def _compute_embedding(self, model):
        ...
        #     img_mu_feat, sample_img_feats, fix_img_embedding = model.new_encode_image(images)
        #     model.cross_former(img_mu_feat, fix_img_embedding, fix_img_embedding, ori_caption_ids,images, img_name) # 均值和固定点嵌入的特征图
        ##########################
        ...
        print(f'文本总耗时{text_time:.6f}s,平均耗时{text_time/qfeats.size(0):.6f}s')
        print(f'图像总耗时{image_time:.6f}s,平均耗时{image_time/gfeats.size(0):.6f}s')

        # total_time = img_inference_time_ms + text_inference_time_ms
        ...
        return qfeats.cuda(), gfeats.cuda(), qids, gids  # 先去除概率相似度仅用于训练更快
    ...
    def eval(self, model, i2t_metric=False):
        ...
        #     'gids': gids
        # }
        # torch.save(tensor_dict, 'rstp_for_tsne.pth')
