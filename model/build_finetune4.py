...
###### 热力图代码 #####
    def show_mask_on_image(self, img, mask):
        ...
        # Apply color map to the mask to create a heatmap
        heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
        heatmap = np.float32(heatmap) / 255.0  # Normalize heatmap
    
        # Overlay the heatmap on the original image
        # cam = heatmap.transpose(2, 0, 1).reshape(1, 3, 384, 128) + np.float32(img)
        heatmap = heatmap * 0.3
        cam = heatmap + img 
        cam = cam / np.max(cam)  # Normalize combined image to range [0, 1]
    
        # Convert to uint8 and return
        return np.uint8(255 * cam)

    def cross_former_for_heatmap(self, q, k, v, caption_ids, ori_img, img_name):
        # Compute cross-attention and retrieve attention weights
        q = q.half()
        k = k.half()
        v = v.half()

        k = k[:,1:,:] # 全部 Patch
        q = F.normalize(q, p=2, dim=-1)  # [b, 1, 512]
        k = F.normalize(k, p=2, dim=-1)  # [b, 192, 512]
        attns  = torch.matmul(q, k.transpose(1, 2)).squeeze(1)
        attns = F.softmax(attns, dim=-1)
        attns_class_token = attns

        import cv2
        # Compute the mean attention, normalize, and resize to match the input image size

        for i in range(attns_class_token.shape[0]):
            mean_attn_i = attns_class_token[i].reshape(24,8).detach().cpu().numpy()
            mean_attn_i = (mean_attn_i - mean_attn_i.min()) / (mean_attn_i.max() - mean_attn_i.min())
            mask = cv2.resize(mean_attn_i.astype(np.float32), (128,384))

            # Define mean and standard deviation for denormalizing the original image
            ...
            original_img = ori_img[i].detach().cpu() * std + mean
            original_img = original_img.permute(1, 2, 0).numpy()
            original_img = np.clip(original_img, 0, 1)
            original_img = np.uint8(255 * original_img)

            # Overlay the mask on the original image
            img_array = self.show_mask_on_image(original_img, mask)

            # Rearrange the dimensions for display purposes
            # img_array = np.transpose(mask[0], (1, 2, 0))

            import os
            save_path = os.path.join("mean_heatmap", img_name[i]+".jpg")
    ...
    def forward(self, batch):
        ...
        if 'crsr' in self.current_task: 
            ...
            for i in range(self.sample_num):
                image_feats_i = image_embeds[:,i,:,:]  # 采样点第i个  [b,193,512]
                with autocast():
                    recover_i = self.cross_former(mlm_text_mu, image_feats_i, image_feats_i) # 图像采样点的class token重建text的细粒度特征
