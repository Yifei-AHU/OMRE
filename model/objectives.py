...
def cal_simi_v4(img_mu, text_mu, image_fetures, text_fetures, a):  
    ...
    # 融合相似度
    # i2t_simi = torch.log(torch.exp(a * i2t_m2s_simi).reshape(img_batch_size,text_batch_size,sample_num).sum(dim=-1) + torch.exp(a * i2t_s2m_simi).reshape(img_batch_size,sample_num,text_batch_size).sum(1) + torch.exp(a * i2t_m2m_simi)) / (a*(sample_num + 1))
    # t2i_simi = torch.log(torch.exp(a * t2i_m2s_simi).reshape(text_batch_size,img_batch_size,sample_num).sum(dim=-1) + torch.exp(a * t2i_s2m_simi).reshape(text_batch_size,sample_num,img_batch_size).sum(1) + torch.exp(a * t2i_m2m_simi)) / (a*(sample_num + 1))
    i2t_simi = torch.log(torch.exp(a * i2t_m2s_simi).reshape(img_batch_size,text_batch_size,sample_num).sum(dim=-1) + torch.exp(a * i2t_s2m_simi).reshape(img_batch_size,sample_num,text_batch_size).sum(1)) / (a*(sample_num))
    t2i_simi = torch.log(torch.exp(a * t2i_m2s_simi).reshape(text_batch_size,img_batch_size,sample_num).sum(dim=-1) + torch.exp(a * t2i_s2m_simi).reshape(text_batch_size,sample_num,img_batch_size).sum(1)) / (a*(sample_num))
    return i2t_simi, t2i_simi

def sdm_v4(img_mu, text_mu, image_fetures, text_fetures, pid, logit_scale, a, image_id=None, factor=0.3, epsilon=1e-8):  
    ...
    # 融合相似度
    # i2t_simi = torch.log(torch.exp(a * i2t_m2s_simi).reshape(batch_size,batch_size,sample_num).sum(dim=-1) + torch.exp(a * i2t_s2m_simi).reshape(batch_size,sample_num,batch_size).sum(1) + torch.exp(a * i2t_m2m_simi)) / (a*(sample_num + 1))
    # t2i_simi = torch.log(torch.exp(a * t2i_m2s_simi).reshape(batch_size,batch_size,sample_num).sum(dim=-1) + torch.exp(a * t2i_s2m_simi).reshape(batch_size,sample_num,batch_size).sum(1) + torch.exp(a * t2i_m2m_simi)) / (a*(sample_num + 1))
...
def cal_simi_v5(img_mu, text_mu, image_fetures, text_fetures):  
    img_batch_size = image_fetures.size(0)
    text_batch_size = text_fetures.size(0)
    sample_num = image_fetures.shape[1] # image_fetures-- [b,num,512]
