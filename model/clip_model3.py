...
#     def forward(self, x: torch.Tensor):
#         x1 = x + self.attention(self.ln_1(x))
#         x = self.mlp(x + self.ln_2(x1)) # 300-301 new_2_4
#         x = self.linear(x)
#         return x
...
#         self.mu_resblocks = ResidualAttentionBlock(width, heads, attn_mask)
#         # self.logsigma_resblocks = ResidualAttentionBlock(width, heads, attn_mask) # sigma_1
#         self.logsigma_resblocks = ResidualAttentionBlock2(width, heads, attn_mask) # sigma_2
#         self.type = type
...
#         x = self.resblocks(x)
#         mu = self.mu_resblocks(x)
#         logsigma = self.logsigma_resblocks(x)
#         return mu, logsigma
...
#         x, logsigma = self.transformer(x,modal) # modal==None
#         x = x.permute(1, 0, 2)  # LND -> NLD
#         logsigma = logsigma.permute(1,0,2)

#         x = self.ln_post(x)

#         if self.proj is not None:
#             x = x @ self.proj
#             # logsigma = logsigma @ self.proj 
#         return x, logsigma
...
#         self.initialize_parameters()  # 不进行初始化
...
#         x = x.permute(1, 0, 2)  # NLD -> LND
#         x, sigma = self.transformer(x, modal) # modal =None
#         x = x.permute(1, 0, 2)  # LND -> NLD
#         sigma = sigma.permute(1, 0, 2)
#         x = self.ln_final(x).type(self.dtype)
#         x = x @ self.text_projection
#         # sigma = sigma @ self.text_projection
#         return x, sigma
    
#     def encode_ori_text(self, text, modal=None):
#         x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]

#         x = x + self.positional_embedding.type(self.dtype)
#         x = x.permute(1, 0, 2)  # NLD -> LND
#         x, _ = self.transformer(x, modal) # modal =None
#         x = x.permute(1, 0, 2)  # LND -> NLD
#         x = self.ln_final(x).type(self.dtype)
#         x = x @ self.text_projection
#         return x
...
#             image_mu, img_sigma = self.encode_image(image,modal='visual')
#             text_mu, text_sigma = self.encode_text(text,modal='text')
#             ori_text = self.encode_ori_text(ori_text, modal='text')
#         else:
#             image_mu, img_sigma = self.encode_image(image)
#             text_mu, text_sigma = self.encode_text(text)
#             ori_text = self.encode_ori_text(ori_text)

#         return image_mu, img_sigma, text_mu, text_sigma, ori_text
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.width = width
        self.layers = layers # 12
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers-1)])  # 直接完整12层
        self.mu_resblocks = ResidualAttentionBlock(width, heads, attn_mask)
        self.logsigma_resblocks = ResidualAttentionBlock2(width, heads, attn_mask) # sigma_2
        self.type = type
    def forward(self, x, modal=None): 
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, modal=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        fix_embedding, x, logsigma = self.transformer(x,modal) # modal==None
        x = x.permute(1, 0, 2)  # LND -> NLD
        logsigma = logsigma.permute(1,0,2)
        fix_embedding = fix_embedding.permute(1, 0, 2)

        x = self.ln_post(x)
        fix_embedding = self.ln_post(fix_embedding)

        if self.proj is not None:
            x = x @ self.proj
            fix_embedding = fix_embedding @ self.proj
        return fix_embedding, x, logsigma
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, modal=None):
        ...
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        fix_embedding, x, sigma = self.transformer(x, modal) # modal =None
        x = x.permute(1, 0, 2)  # LND -> NLD
        sigma = sigma.permute(1, 0, 2)
        fix_embedding = fix_embedding.permute(1, 0, 2)
        x = self.ln_final(x).type(self.dtype)
        ...
        fix_embedding = self.ln_final(fix_embedding).type(self.dtype)
        fix_embedding = fix_embedding @ self.text_projection

        return fix_embedding, x, sigma
    ...
    def encode_ori_text(self, text, modal=None):
        ...
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        _, x, _ = self.transformer(x, modal) # modal =None
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)
        x = x @ self.text_projection
        return x
    ...
    def forward(self, image, text, ori_text):
        if text.size(0)==2*image.size(0):
            fix_img_embedding, image_mu, img_sigma = self.encode_image(image,modal='visual')
            fix_text_embedding, text_mu, text_sigma = self.encode_text(text,modal='text')
            ori_text = self.encode_ori_text(ori_text, modal='text')
        else:
            fix_img_embedding, image_mu, img_sigma = self.encode_image(image)
            fix_text_embedding, text_mu, text_sigma = self.encode_text(text)
            ori_text = self.encode_ori_text(ori_text)

        return image_mu, img_sigma, text_mu, text_sigma, ori_text, fix_img_embedding, fix_text_embedding
